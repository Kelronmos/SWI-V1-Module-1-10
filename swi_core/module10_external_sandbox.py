...
timeout and a CPU/memory resource limit (via `resource` on POSIX systems),
capturing stdout/stderr and the exit status rather than executing it inline
in the calling process. A runaway or hanging snippet is killed at the
...
class ExternalSandbox:
    """Module 10: subprocess-isolated execution with timeout and resource caps."""
    ...
    def run(self, source_code: str) -> SandboxResult:
        ...
        try:
            ...
            return SandboxResult(
                stdout=e.stdout or "", stderr=(e.stderr or "") + "\n[SANDBOX] timed out", exit_code=-1, timed_out=True
            )
