...
Heuristic: skip lines that discuss the phrase in a limiting, negative, or
definitional context. Historical VOLUME_* manuals are excluded.
"""
...
def _line_is_safe(line: str) -> bool:
    ...
    if any(m in lower for m in SAFE_MARKERS):
        return True
    if lower.startswith("- ") and lower.rstrip(";:.").lstrip("- ") in FORBIDDEN_AS_CURRENT:
        return True
